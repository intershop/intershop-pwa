...
const DIST_FOLDER = process.env.DIST_FOLDER || join(process.cwd(), 'dist', process.env.THEME || '');

const BROWSER_FOLDER = process.env.BROWSER_FOLDER || join(DIST_FOLDER, 'browser');
...
export async function run() {
  ...
  const ssrServer = http.createServer(app(mainServer)).listen(PORT);
