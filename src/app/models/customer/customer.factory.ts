...
export class CustomerFactory {
  ...
  static fromFormToData(form: FormGroup) {
    ...
    FactoryHelper.primitiveMapping<FormGroup, CustomerData>(form.value, customerData);
    ...
    if (form.get('credentials')) {
      customerData.credentials = CredentialsFactory.fromFormToData(<FormGroup>form.get('credentials'));
      customerData.email = customerData.email || customerData.credentials.login;
    }

    // create and assign a new address object
    if (form.get('address')) {
      customerData.address = AddressFactory.fromFormToData(<FormGroup>form.get('address'));
