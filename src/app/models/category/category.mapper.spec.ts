...
describe('CategoryFactory', () => {
  describe('fromData', () => {
    ...
    it(`should throw error when getting no RawCategory`, () => {
      expect(function() { CategoryMapper.fromData(null); }).toThrow();
    });

    it(`should handle Subcategories on RawCategory`, () => {
      const category: Category = CategoryMapper.fromData({ id: '1', subCategories: [{ id: '2' } as CategoryData], subCategoriesCount: 1 } as CategoryData);
