...
import { routerTestNavigatedAction, routerTestNavigationAction } from 'ish-core/utils/dev/routing';
...
describe('Orders Effects', () => {
  ...
  describe('cancelOrderAfterRedirectAbortion$', () => {
    ...
    it('should navigate to the payment page with cancel parameters when the browser back button returns to the cart page', fakeAsync(() => {
    ...
    it('should not navigate to the payment page on ordinary navigation with a pending order', fakeAsync(() => {
      when(orderServiceMock.getPendingPaymentRedirectOrderId()).thenReturn(order.id);
    ...
    it('should navigate to the payment page if the page is restored from the back/forward cache with a pending order', fakeAsync(() => {
      router.navigateByUrl('/checkout/review');
      tick(500);

      // no pending order yet, so the initial navigation must not trigger a cancellation
      effects.cancelOrderAfterRedirectAbortion$.subscribe({ next: noop, error: fail });
      tick(500);
    ...
    it('should navigate to the payment page after a non-BFCache history reload triggers the first router navigation with a pending order', fakeAsync(() => {
      ...
      try {
        ...
        // the effect evaluates isHistoryReload() at construction time, so build a fresh instance after mocking
        const historyReloadEffects = new OrdersEffects(
          new Actions(of(routerTestNavigationAction({ routerState: { url: '/checkout/review' } }))),
        ...
        historyReloadEffects.cancelOrderAfterRedirectAbortion$.subscribe({ next: noop, error: fail });
    ...
    it('should not do anything if the provider redirected back on its own with query params', fakeAsync(() => {
      when(orderServiceMock.getPendingPaymentRedirectOrderId()).thenReturn(order.id);
  ...
  describe('cleanupRedirectMarker$', () => {
    it('should clear the pending order marker via the order service', done => {
      actions$ = of(loadBasketSuccess({ basket: { id: 'BID' } as Basket }));
  ...
  describe('processPaypalOrderCreation$', () => {
    ...
    describe('startPaypalOrderCreation$', () => {
      it('should call orderService.createOrder with basketId', done => {
        ...
        const action = processPaypalOrderCreation({});
        actions$ = of(action);

        effects.startPaypalOrderCreation$.subscribe(() => {
      ...
      it('should dispatch emitPaypalOrderId action on success', () => {
        ...
        expect(effects.startPaypalOrderCreation$).toBeObservable(expected$);
      });

      it('should dispatch createOrderFail and emitPaypalOrderId actions on error', done => {
        when(orderServiceMock.createOrder(anything(), anything())).thenReturn(
          throwError(() => makeHttpError({ message: 'error' }))
        );
        const action = processPaypalOrderCreation({});
        ...
        effects.startPaypalOrderCreation$.pipe(take(2), toArray()).subscribe(actions => {
          expect(actions).toHaveLength(2);
          expect(actions[0]).toEqual(createOrderFail({ error: makeHttpError({ message: 'error' }) }));
          expect(actions[1]).toEqual(emitPaypalOrderId({ orderStatus: 'ERROR' }));
          done();
        });
      });
    });
    ...
    describe('continuePaypalOrderCreation$', () => {
      it('should call orderService.continueOrderCreation with orderId', done => {
        ...
        effects.continuePaypalOrderCreation$.subscribe(() => {
      ...
      it('should dispatch createOrderSuccess and emitPaypalOrderId actions on success', done => {
        ...
        effects.continuePaypalOrderCreation$.pipe(toArray()).subscribe(actions => {
          expect(actions).toHaveLength(2);
      ...
      it('should dispatch cancelPaypalOrderCreation and emitPaypalOrderId actions when order is cancelled', done => {
        const cancelledOrder = {
          ...
          orderCreation: {
            ...orderWithPaypalRedirect.orderCreation,
            status: 'ROLLED_BACK',
          },
        } as Order;
      ...
      it('should dispatch createOrderFail action on error', done => {
