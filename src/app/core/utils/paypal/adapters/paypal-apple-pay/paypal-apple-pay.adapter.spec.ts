...
describe('Paypal Apple Pay Adapter', () => {
  ...
  describe('onApplePayButtonClicked()', () => {
    ...
    it('should call processPaypalOrderCreation on checkout facade', async () => {
      await adapter.testOnApplePayButtonClicked();

      verify(checkoutFacade.processPaypalOrderCreation()).once();
    });

    it('should create ApplePaySession with correct parameters', async () => {
      await adapter.testOnApplePayButtonClicked();

      expect((window as any).ApplePaySession).toHaveBeenCalledWith(4, expect.any(Object));
    });

    it('should call session.begin()', async () => {
      await adapter.testOnApplePayButtonClicked();

      expect(mockApplePaySession.begin).toHaveBeenCalled();
    });

    it('should abort session in onvalidatemerchant when orderStatus is ERROR', async () => {
      const errorSpy = jest.spyOn(console, 'error').mockImplementation();
      when(paypalDataTransferService.paypalOrder$).thenReturn(
        of({ orderId: 'ICM_ORDER_123', paypalOrderId: 'PAYPAL_ORDER_456', orderStatus: 'ERROR' })
      );
      // Re-inject adapter to pick up new mock
      adapter = TestBed.inject(TestablePaypalApplePayAdapter);
      adapter.setApplePayConfig(mockApplePayConfig);
      ...
      // Session is created synchronously – trigger onvalidatemerchant to exercise error path
      await mockApplePaySession.onvalidatemerchant({ validationURL: 'https://apple.com/validate' });

      expect((window as any).ApplePaySession).toHaveBeenCalled();
      expect(mockApplePaySession.begin).toHaveBeenCalled();
      expect(mockApplePaySession.abort).toHaveBeenCalled();
      expect(errorSpy).toHaveBeenCalledWith(
        'Order creation failed or no PayPal order ID available, aborting Apple Pay session'
      );
