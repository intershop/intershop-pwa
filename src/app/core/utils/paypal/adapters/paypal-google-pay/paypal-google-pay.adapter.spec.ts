...
import { BehaviorSubject, noop, of, take, throwError } from 'rxjs';
...
import { GooglePayPaymentAuthorizationResult } from 'ish-core/utils/paypal/paypal-model/paypal.model';
...
class TestablePaypalGooglePayAdapter extends PaypalGooglePayAdapter {
  ...
  setPaypalOrderId(orderId: string): void {
    (this as any).paypalOrderId = orderId;
  }
...
describe('Paypal Google Pay Adapter', () => {
  ...
  beforeEach(() => {
    ...
    when(paypalDataTransferService.paypalOrder$).thenReturn(
      of({ orderId: 'ICM_ORDER_123', paypalOrderId: 'PAYPAL_ORDER_456' })
    );
  ...
  describe('renderGooglePayButton()', () => {
    ...
    it('should use TEST environment as default when googlePayEnvironment is not set', async () => {
      when(appFacade.paypalClientConfig$()).thenReturn(of({ googlePayEnvironment: undefined }));
  ...
  describe('onGooglePayButtonClicked()', () => {
    ...
    it('should call processPaypalOrderCreation with orderId when user cancels', async () => {
      ...
      adapter.setOrderId('ICM_ORDER_123');
      adapter.setGooglePayConfig(mockGooglePayConfig);
      adapter.setGooglePaymentClient(mockGooglePaymentClient);
  ...
  describe('onPaymentAuthorizedCallback()', () => {
    ...
    beforeEach(() => {
      ...
      adapter.setPaypalOrderId('PAYPAL_ORDER_456');
      adapter.setOrderId('ICM_ORDER_123');
    });
