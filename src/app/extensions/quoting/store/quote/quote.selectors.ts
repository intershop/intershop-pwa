...
export const getCurrentQuotes = createSelector(selectAll, quotes => quotes.map(addStateToQuote));

/**
 * Select the selected quote with the appended product data if available.
 */
export const getSelectedQuoteWithProducts = createSelector(getSelectedQuote, getProductEntities, (quote, products) =>
  !quote
    ? undefined
    : {
        ...quote,
