...
describe('Account Addresses Component', () => {
    ...
    it('should display only one preferred address if preferred invoice and shipping address are equal', () => {
      ...
      expect(
        element.querySelectorAll(
          'div[data-testing-id=preferred-invoice-and-shipping-address] formly-group formly-field'
        )
      ).toHaveLength(2);
  ...
  describe('enabled address doctor feature', () => {
    ...
    beforeEach(() => {
      ...
      when(featureEventService.sendNotification('addressDoctor', 'check-address', anything())).thenReturn(
        'custom-process-id'
      );
      when(featureEventService.eventResultListener$('addressDoctor', 'check-address', 'custom-process-id')).thenReturn(
        of({
          id: 'custom-process-id',
          event: 'check-address-successful',
          successful: true,
          data: mockAddresses[0],
        } as FeatureEventResult)
      );
    });
    ...
    it('should emit createCustomerAddress event when createCustomerAddress is triggered', () => {
      ...
      verify(featureEventService.sendNotification('addressDoctor', 'check-address', anything())).once();
      verify(accountFacade.createCustomerAddress(anything())).once();
    });
