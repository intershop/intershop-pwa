...
> = {
  ...
  create: context => {
    ...
    return {
      'Identifier[name=/(toBe|toEqual)/]'(node: TSESTree.Identifier) {
        const expectStatement = getClosestAncestorByKind(context, AST_NODE_TYPES.CallExpression);
        const expectStatementText = context.getSourceCode().getText(expectStatement);
        mergedReplacements.forEach(rep => {
          const match = expectStatementText.match(rep.pattern)?.[0];
          if (match) {
            context.report({
              node,
              ...
              fix: fixer =>
                fixer.replaceTextRange(
                  expectStatement.range,
                  expectStatementText.replace(rep.pattern, rep.replacement)
                ),
            });
          }
        });
