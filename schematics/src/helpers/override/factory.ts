...
export function override(options: Options): Rule {
  return async host => {
    ...
    if (options.html) {
      const target = from.replace(/\.component.*/, `.component.${options.theme}.html`);
    ...
    if (options.scss) {
      const originalScss = from.replace(/\.component.*/, `.component.scss`);
      ...
      const target = from.replace(/\.component.*/, `.component.${options.theme}.scss`);
      host.create(target, `/* style definitions for overriding with theme "${options.theme}" */`);
    }

    if (options.ts) {
      const target = from.replace(/\.component.*/, `.component.${options.theme}.ts`);
