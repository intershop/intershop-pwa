...
However, for small forms or optional secondary actions, disabling the button until the input is valid can be appropriate.
...
- Disabling buttons for small or optional forms ensures that users cannot submit invalid data while keeping the experience predictable.
