...
This builds all active themes; each container starts the theme selected by `THEME`.
The supplied Docker Compose configuration runs one container per theme and routes requests through nginx.
...
Use `ps` to find the SSR process ID, then send the signal with `kill -USR2 <process_id>`.
