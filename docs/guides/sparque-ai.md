...
- **serverUrlSsr**: the (internal) URL of the SPARQUE server that the PWA SSR will connect to when running in the same Kubernetes cluster (optional, if not provided `serverUrl` is used).
