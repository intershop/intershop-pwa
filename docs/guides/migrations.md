...
The structure of the elements that contained the `ngbNav` directive was updated to ensure that it is compliant with accessibility standards.
It was previously based on an `<ul>` with `<li role="presentation">` elements, but now it uses a `<nav>` with `<ng-container>` elements.
