...
- Form validation:
  ...
  - If the user presses the submit button, all form controls of the form are validated. In case of validation errors, the focus is set to the first invalid form field. The submit button will be disabled as long as there is any unhandled form error. On large forms (with more than 8 form fields) the disabled button remains interactive and focusable to provide a descriptive help text and screen reader-only content.
