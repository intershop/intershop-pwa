...
Using the external URL defined in `ICM_BASE_URL` will not work in joint kubernetes deployments.

There might be cases where the internal port of the PWA NGINX container does not fit with the port used by the PWA on the outside, e.g. behind an Ingress that takes care of the SSL termination or in deployments where the PWA is mapped to a different external port.
In such cases the port forwarded to the Responsive Starter Store will not match the PWAs external port resulting in wrong URLs generated by the Responsive Starter Store.
To solve this, the `HYBRID_FORWARDED_PORT` can be set as an environment variable of the NGINX container to the external port used to access the PWA.
...
> **Only for development environments**
>
> It might be necessary to set `TRUST_ICM=1` if the used development ICM is deployed with an insecure certificate or a certificate that does not work for the PWA parts.
