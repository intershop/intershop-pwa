...
test('validates the active multi-build subset', () => {
  const workspace = createWorkspace();

  assert.deepEqual(resolveActiveThemes(workspace, 'b2b, acme'), ['b2b', 'acme']);
  assert.throws(() => resolveActiveThemes(workspace), /No active themes configured/);
  assert.throws(() => resolveActiveThemes(workspace, 'b2b,missing'), /Unknown active themes: missing/);
});

test('completes full and client-only configuration lists', () => {
  assert.deepEqual(completeBuildConfigurations(['acme'], false), ['acme', 'production', 'ssr']);
  assert.deepEqual(completeBuildConfigurations(['acme', 'production'], false), ['acme', 'production', 'ssr']);
  assert.deepEqual(completeBuildConfigurations(['acme', 'development'], false), ['acme', 'development', 'ssr']);
  assert.deepEqual(completeBuildConfigurations(['acme', 'development', 'ssr'], false), ['acme', 'development', 'ssr']);
  assert.deepEqual(completeBuildConfigurations(['acme'], true), ['acme', 'production']);
  assert.deepEqual(completeBuildConfigurations(['acme', 'development', 'ssr'], true), ['acme', 'development']);
});
