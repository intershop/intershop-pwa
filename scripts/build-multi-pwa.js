...
if (processArgs.includes('client') || !processArgs.includes('server'))
  clientBuilds.push(
    ...configurations.map(({ theme }) =>
      `build client --configuration=${theme},production -- --output-path=dist/${theme}/browser ${extraArgs}`.trim()
    )
  );
...
if (processArgs.includes('server') || !processArgs.includes('client'))
  serverBuilds.push(
    ...configurations.map(({ theme }) =>
      `build server --configuration=${theme},production -- --output-path=dist/${theme}/server ${extraArgs}`.trim()
    )
  );
